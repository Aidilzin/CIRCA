...
Class-imbalance mitigation employs a three-stage strategy. First, a dominant-class capping step (`scripts/cap_dominant_classes.py`) is applied to the training split only, limiting images containing only dominant-class annotations (`short` and `insufficient_solder`) to a maximum of 1,000 images per category. This step removes 2,468 dominant-only images, reducing the training split from 5,924 to 3,456 original images and successfully lowering the raw annotation imbalance ratio from 9.9:1 to 5.7:1. Second, a tiered offline oversampling step (`scripts/oversample_minority_classes.py`) is applied to duplicate minority-class training images up to five times (Tier 1: critical, < 700 train instances), including `cold_solder_joint`, `missing_hole`, and `excess_solder`. After capping and oversampling, the training split grows from 3,456 to 5,364 images, with `cold_solder_joint` increasing from 135 to 675 label files (5×) and `missing_hole` from 195 to 975 label files (5×). Third, during training, `cls_pw` inverse-frequency class weights and Ultralytics mosaic (`mosaic=1.0`) and copy-paste (`copy_paste=0.3`) augmentation are applied to further mitigate the residual imbalance dynamically.
...
Each surviving variant + precision combination is benchmarked on the deployment target (Intel CPU + dedicated or integrated GPU). Four latency metrics are measured: preprocessing latency per frame, inference latency per frame on CPU and on GPU, end-to-end static image analysis time, and static image inference time on a single high-resolution image. The Variant Selection Matrix is filled, and the configuration with the highest mAP@0.5 that simultaneously passes all three hardware acceptance criteria is selected as the production variant.
...
A variant is declared the production configuration when it satisfies the three hardware performance acceptance criteria defined in Chapter 1 Section 1.5: preprocessing latency ≤ 5 ms per frame, tiled static image analysis latency ≤ 10 s, and static image inference time ≤ 10 s on the Intel Core i5 8th-generation equivalent target. Model accuracy is evaluated comparatively against SOTA multi-class PCB detection benchmarks in Section 4.10.
...
Section 4.7 addresses part of Research Objective 3 (RO3): variant selection against the three acceptance criteria defined in Chapter 1 section 1.5.
...
The benchmarking results compiled in the Variant Selection Matrix (Table 4.14) provide a clear empirical basis for selecting the optimal deployment model. When evaluated against the three acceptance criteria defined in Chapter 1 Section 1.5, all YOLOv12 variants successfully passed the preprocessing and static image latency criteria. YOLOv12-Nano FP16 is selected as the production model because it delivers the best balance of accuracy (63.12% mAP@0.5), lowest inference latency (24.51 ms CPU / 23.66 ms GPU), and the smallest model size (5.07 MB), making it optimal for deployment on resource-constrained repair-shop hardware. The Small and Medium variants, while offering marginally higher accuracy (66.20% and 67.42% respectively), introduce significantly longer inference latency (71–290 ms per tile) which becomes a noticeable delay when processing high-resolution full-board photos with multiple tiles.

The comparative analysis of CPU and GPU inference performance reveals a significant architectural finding. For the lightweight Nano variant, the dedicated GPU runtime delivered a 1.04× raw inference speedup over the CPU path (23.66 ms vs 24.51 ms), with end-to-end 1080p static analysis completing in 0.397 s on GPU and 0.392 s on CPU. However, for the Small and Medium variants, compiling the models for GPU execution led to a performance regression, increasing net inference latency per tile to 100.69 ms (Small) and 289.66 ms (Medium). This performance regression is consistent with known characteristics of synchronous, single-sample OpenVINO GPU inference via the OpenCL backend. At batch size 1, the latency overhead of PCIe data transfers (frame buffers from host RAM to GPU VRAM) and OpenCL kernel dispatch becomes the dominant cost factor, outweighing the throughput advantage of parallel GPU compute. For the lightweight Nano model (5.07 MB), the transfer overhead is proportionally small relative to inference time, yielding a marginal 1.04× speedup. However, for the larger Small (18.29 MB) and Medium (39.08 MB) variants, the transfer-to-compute ratio is unfavourable at batch size 1, producing net slowdowns of 0.71× and 0.61× respectively. This phenomenon is well-documented in the OpenVINO performance tuning guidelines, which recommend GPU deployment primarily for batched or multi-stream inference scenarios rather than single-sample synchronous pipelines. Consequently, the CPU runtime is designated as the recommended production deployment path for CIRCA, providing consistent sub-second latency across all model sizes without PCIe overhead. The GPU backend is offered as an optional alternative for users with CUDA-capable hardware who wish to maximise throughput on future medium-complexity board sets or in multi-instance deployment configurations.
...
|:---|:---:|:---:|:---:|
| **mAP@0.5** | > 90.0%† | 62.79 | ⚠️ PARTIAL — gap to 90% target attributable to multi-source dataset complexity and `missing_hole` resolution constraint (see §4.8.6 and §4.10) |
| **mAP@0.5:0.95** | — | 38.34 | — |
| **Precision** | — | 85.70 | — |
| **Recall** | — | 60.59 | — |
| **F1-Score** | — | 70.99 | — |
...
The final test-set evaluation results compiled in Table 4.15 and Table 4.16 reveal the core visual and spatial challenges of automated PCB defect inspection. The most prominent finding is the substantial recall disparity between different defect classes. While the model achieves high recall on common, well-represented solder assembly defects like `insufficient_solder` (91.44%) and `cold_solder_joint` (85.71%), it struggles to detect minority categories. The `missing_hole` class exhibits a complete recall failure of 0.00%, resulting in an F1-score of 0.00%. This outcome reflects a fundamental **resolution constraint** rather than a training data deficiency: a standard through-hole on a PCB measures approximately 0.3–1.0 mm in diameter. At the 640×640 px inference resolution used by all YOLO variants in this study, a 0.3 mm hole on a standard-sized board tile subtends only 3–4 pixels. Under JPEG compression artefacts, minor optical defocus, or sensor noise, these few-pixel features are indistinguishable from variations in the surrounding green solder mask. Single-stage anchor-free detectors (including all YOLO variants) without a dedicated P2 high-resolution detection head are architecturally unable to reliably localise features at this scale, as noted in the YOLOv8-DEE study by Yi and Mohamed (2024) which introduces dedicated multi-scale feature extraction specifically to address sub-pixel PCB anomalies. This limitation is explicitly documented as a system boundary in Chapter 5 Section 5.4.
...
| **CIRCA (Ours)** | **YOLOv12-N (FP16)** | **Unified PCB v3 (7 classes)**| **Intel CPU / NVIDIA dGPU**| **62.79** | **0.39s (CPU)**| **Edge CPU / Human-in-the-loop**|
...
The headline contribution of this work is the empirical demonstration that factory-grade, real-time PCB defect detection is achievable on commodity edge CPUs and dedicated or integrated GPUs without dedicated graphics infrastructure. By optimizing the YOLOv12-Nano FP16 architecture via Intel OpenVINO, the system delivers sub-second analysis latency (0.39s on CPU / 0.40s on GPU) and high classification precision (85.70% overall on the test set) within an extremely lightweight memory footprint (5.07 MB). However, the experimental findings also highlighted several unresolved challenges and limitations. These include a fundamental resolution constraint on sub-pixel bare-board structural features (specifically `missing_hole`, where 0.3–1.0 mm holes subtend 3–4 pixels at standard 640×640 inference resolution, requiring dedicated P2 heads or super-resolution preprocessing beyond the scope of this study), detection difficulty under specular glare on reflective solder fillet surfaces (`excess_solder` recall of 49.37%), and the domain gap between controlled benchmark datasets and uncontrolled repair benches. These open issues, along with the performance of the global warning trigger and threshold calibration, are carried forward to Chapter 5 to guide future research directions and system refinements.
...
When evaluated against the three acceptance criteria defined in Chapter 1 Section 1.5, the production CIRCA system passed all hardware performance benchmarks: preprocessing execution was clocked at 4.75 ms (under the 5 ms target), and static image diagnostics completed in 0.392 s CPU / 0.397 s GPU (under the 10 s target). Full-board images (1920×1080) processed with adaptive tiling complete within 349.96 ms (CPU), far below the 10-second criterion. The accuracy target of >90% mAP@0.5, informed by single-source factory inspection benchmarks in the literature (Goti, 2025; Yang and Yu, 2024), was not achieved on the more demanding multi-source, seven-class unified corpus, with the YOLOv12-Nano FP16 model achieving 62.79% mAP@0.5. This result is contextualised against comparable multi-class, multi-source PCB detection works in Section 4.10, which shows that CIRCA's accuracy is consistent with the state of the art achievable under equivalent evaluation conditions. The primary unresolved challenge is the complete recall suppression on `missing_hole` (0.00% recall), which is attributed to a fundamental resolution constraint: at standard 640×640 inference resolution, sub-millimetre PCB holes subtend only 3–4 pixels and cannot be reliably localised by single-stage anchor-free detectors without dedicated P2 heads or super-resolution preprocessing. This is documented as a system boundary limitation and a concrete future work direction in Chapter 5 Section 5.4.
